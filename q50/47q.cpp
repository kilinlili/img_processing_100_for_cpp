...
//膨張
//四方に画素値255が一つでもあったら注目画素も255にする．

//cv::Mat morphology_dilation(cv::Mat src, cv::Mat out)
void morphology_dilation(cv::Mat src, cv::Mat &dst)
{
    ...
    out = src.clone();
     ...
     for (int y = 0; y < src.rows; y++){
      for (int x = 0; x < src.cols; x++){
        // check left pixel
        if ((x > 0) && (src.at<uchar>(y, x - 1) == 255)){
          out.at<uchar>(y, x) = 255;
          continue;
        } 

        // check up pixel
        if ((y > 0) && (src.at<uchar>(y - 1, x) == 255)){
          out.at<uchar>(y, x) = 255;
          continue;
        }

        // check right pixel
        if ((x < src.cols - 1) && (src.at<uchar>(y, x + 1) == 255)){
          out.at<uchar>(y, x) = 255;
          continue;
        }

        // check left pixel
        if ((y < src.rows - 1) && (src.at<uchar>(y + 1, x) == 255)){
          out.at<uchar>(y, x) = 255;
          continue;
        }
      }
...
int main(int argc, char *argv[])
{
    ...
    //cv::Mat hoge = morphology_dilation(gray, zero);
    morphology_dilation(gray, zero);//参照渡しミス．
    //zero = Morphology_dia(gray, 1);
    cv::imshow("sample", zero);
    cv::waitKey(0);

    cv::imwrite("./img/47q_ans.jpg", zero);

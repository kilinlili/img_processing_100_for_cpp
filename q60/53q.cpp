...
void morphology_dilation(cv::Mat &src, cv::Mat &dst, int n)
{
    ...
    for(int i = 0; i < n; i++){
        ...
        //cv::imwrite("./img/" + std::to_string(i) + ".jpg", dst);
        dst_base = dst.clone();
    }
}
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    //クロージング
    morphology_dilation(otu, dilation, 3);//膨張
    cv::imshow("sample", dilation);
    cv::waitKey(0);
    morphology_erosion(dilation, erosion, 3);//収縮
    cv::imshow("sample", erosion);
    cv::waitKey(0);

    cv::Mat result = image_diff(erosion, otu);
    cv::imshow("sample", result);
    cv::waitKey(0);
    cv::imwrite("./img/53q_ans.jpg", result);
